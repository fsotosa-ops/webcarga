...
describe('TripDetailView — hero (la historia del viaje)', () => {
...
describe('TripDetailView — header (IDs unificados + link a TMS)', () => {
  ...
  it('the TMS chip links to the public login page for a TMS-sourced trip', () => {
    renderDetailView({ ...baseTrip, source_system: 'qanalytics' })
  ...
  it('the TMS chip is not a link for a manual trip', () => {
    renderDetailView({ ...baseTrip, source_system: 'manual' })
    expect(screen.queryByTitle(/Abrir en/)).not.toBeInTheDocument()
  })
})

describe('TripDetailView — layout y a11y', () => {
  ...
  it('shows "en el Diario desde" with created_at in the hero', () => {
    renderDetailView({ ...baseTrip, created_at: '2026-06-30 08:00:00' })
  ...
  it('shows Datos operativos always visible, no accordion, without EETT TMS', () => {
    renderDetailView({ ...baseTrip, carrier_name_tms: 'Transportes ACME (texto TMS)' })
...
describe('TripDetailView — Conductor y flota (FleetAssignSection, driver-first)', () => {
  ...
  it('shows the linked carrier as a compact card and unlinks via removeFleetLink', async () => {
    ...
    renderDetailView({ ...baseTrip, carrier_id: 'c1', carrier_name: 'Transportes Sur Spa' })
  ...
  it('shows fuzzy-match candidates by TMS name and picking one fills the draft', async () => {
    ...
    renderDetailView({ ...baseTrip, driver_name_tms: 'HERNANDEZ CONTRERAS EULICES ALFREDO' })
  ...
  it('shows a "create in Empresas" trigger when the TMS name has no fuzzy match', async () => {
    vi.mocked(driversApi.fuzzyMatch).mockResolvedValue([])
    renderDetailView({ ...baseTrip, driver_name_tms: 'NOMBRE SIN CRUCE' })
  ...
  it('does not query fuzzy-match when the trip already has a carrier linked', () => {
    renderDetailView({ ...baseTrip, carrier_id: 'c1', carrier_name: 'Transportes Sur Spa', driver_name_tms: 'ALGUIEN' })
  ...
  it('shows "Sin identificar" when fleet_match_status is UNMATCHED', () => {
    renderDetailView({ ...baseTrip, fleet_match_status: 'UNMATCHED', driver_name_tms: 'ALGUIEN SIN CRUCE' })
  ...
  it('does not show "Sin identificar" when fleet_match_status is MATCHED or unset', () => {
    renderDetailView({ ...baseTrip, driver_name_tms: 'ALGUIEN' })
...
describe('TripDetailView — override de estado', () => {
  ...
  it('reverting the override calls tripsApi.resetField with manual_status', async () => {
    ...
    renderDetailView({ ...baseTrip, manual_status: 'en_seguimiento' })
...
describe('TripDetailView — indicadores (switches, Fase 2 Plan 5)', () => {
  ...
  it('shows explicit override attribution text and a revert control when a field is manually edited', () => {
    renderDetailView({ ...baseTrip, manually_edited_fields: ['is_assigned'], edited_by: 'Felipe Sumadots', edited_at: '2026-07-02 10:15:00' })
...
describe('TripDetailView — Gestión colapsable (página inmersiva, 2026-07-29)', () => {
...
describe('TripDetailView — Bitácora (feed con historial)', () => {
  const note: TripNote = {
  ...
  it('no longer shows a legacy notes/comments block (retirado, Fase 2 Plan 5)', async () => {
    renderDetailView({ ...baseTrip, notes: 'obs vieja', comments: 'comentario viejo' })
    await waitFor(() => expect(tripsApi.listNotes).toHaveBeenCalled())
...
describe('TripDetailView — campos híbridos de fecha (Carga/Desc. Inicio-Fin) — tabla técnica siempre visible', () => {
...
describe('TripDetailView — Ubicación de origen (solo operation_type)', () => {
  it('shows "Sin clasificar" instead of an empty section when origin_operation_type is null', () => {
    renderDetailView({ ...baseTrip, origin_operation_type: null })
...
describe('TripDetailView — motivo de no asignación (Fase 1.5d)', () => {
  ...
  it('hides the reason dropdown once the trip is is_assigned', () => {
    renderDetailView({ ...baseTrip, is_assigned: true }, { meta: metaWithReasons })
    expect(screen.queryByText('Motivo de no asignación')).not.toBeInTheDocument()
  })
})

describe('TripDetailView — focusNotes (badge de bitácora, 2026-07-28)', () => {
  it('scrolls the Bitácora section into view when focusNotes is true', () => {
    const scrollIntoViewMock = vi.fn()
    Element.prototype.scrollIntoView = scrollIntoViewMock

    renderDetailView(baseTrip, { focusNotes: true })
