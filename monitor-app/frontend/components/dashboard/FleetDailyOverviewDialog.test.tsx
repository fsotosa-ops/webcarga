...
import { FleetDailyOverviewDialog } from './FleetDailyOverviewDialog'
...
function renderDialog(props: Partial<Parameters<typeof FleetDailyOverviewDialog>[0]> = {}) {
  const client = new QueryClient({ defaultOptions: { queries: { retry: false } } })
  return render(
    <QueryClientProvider client={client}>
      <FleetDailyOverviewDialog open fecha="2026-08-02" onClose={vi.fn()} onSelectTrip={vi.fn()} {...props} />
    </QueryClientProvider>,
  )
}
...
describe('FleetDailyOverviewDialog', () => {
  it('no renderiza nada cuando open=false', () => {
    renderDialog({ open: false })
    expect(screen.queryByText(/Vista de Flota del Día/)).not.toBeInTheDocument()
  })
