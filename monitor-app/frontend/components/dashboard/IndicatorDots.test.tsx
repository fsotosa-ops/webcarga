...
describe('IndicatorDots', () => {
  ...
  it('toggles off a currently-active indicator', () => {
    ...
    render(<IndicatorDots trip={{ ...baseTrip, is_working: true }} onSaved={vi.fn()} />)
  ...
  it('rolls back optimistic state when PATCH fails, computing correct toggle on second click', async () => {
    ...
    vi.mocked(tripsApi.patch).mockResolvedValueOnce({ ...baseTrip, is_active: true })
    const onSaved = vi.fn()
    render(<IndicatorDots trip={baseTrip} onSaved={onSaved} />)

    // First click: since trip.is_active is false, next = !(false) = true
    fireEvent.click(screen.getByTitle('Activo'))
    expect(tripsApi.patch).toHaveBeenNthCalledWith(1, 't1', { is_active: true })
    ...
    // If rollback had NOT worked, optimistic.is_active would still be true, and next would be !(true) = false
    fireEvent.click(screen.getByTitle('Activo'))

    // Verify the second call computed the correct toggle from the original state
    expect(tripsApi.patch).toHaveBeenNthCalledWith(2, 't1', { is_active: true })
    await waitFor(() => expect(onSaved).toHaveBeenCalledWith(expect.objectContaining({ is_active: true })))
  })
