...
_LOCATION_FIELDS = (
    ...
    "operational_status, is_manual_override, is_origin, created_at, updated_at"
)
...
async def list_locations(
    ...
    origin: str = Query(
        "", description="true = sólo orígenes de ORIGEN (HU-28). No es excluyente con "
            ...
            row = await conn.fetchrow(
                ...
                INSERT INTO public.locations
                    (entity_type, entity_id, site_number, name, country_code, format, address,
                     region_name, region_number, opens_at, closes_at, operation_type, is_origin)
                ...
                body.closes_at, body.operation_type, body.is_origin,
            )
            await log_change(
        ...
        async with conn.transaction():
            ...
            await conn.execute(
                ...
                UPDATE public.locations SET
                    ...
                    is_origin       = COALESCE($15, is_origin),
                    updated_at         = NOW()
                WHERE id = $1
                """,
