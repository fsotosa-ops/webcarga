...
async def list_transporters(
    ...
    offset = (page - 1) * limit
    filter_clause = (
        "AND (business_name ILIKE '%' || $1 || '%' OR rut ILIKE '%' || $1 || '%')"
        if q else ""
    )
    params_list = [q, limit, offset] if q else [limit, offset]
    p1, p2, p3 = ("$1", "$2", "$3") if q else ("", "$1", "$2")

    rows = await pool.fetch(f"""
        SELECT
            id, business_name, rut, account_stage,
