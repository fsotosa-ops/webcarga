...
async def _listar(pool, **kwargs):
    ...
    base = dict(
        ...
        incomplete="", needs_manual_classification="", include_rate="", origin_cd="",
        page=1, limit=50,
    )
    base.update(kwargs)
...
async def test_el_listado_devuelve_is_origin_cd(conexion_revertida):
    ...
    assert "is_origin_cd" in salida["data"][0]
    # Un local nace SIN el rol: el catálogo de CD no se llena solo.
    assert salida["data"][0]["is_origin_cd"] is False
...
async def test_un_lugar_puede_ser_cd_de_origen_y_local_de_entrega_a_la_vez(conexion_revertida):
    ...
    tras_marcar = await patch_location(
        str(creado["id"]), LocationPatchBody(is_origin_cd=True), pool=pool, user=USER,
    )

    assert tras_marcar["is_origin_cd"] is True
...
async def test_el_filtro_origin_cd_recorta_el_listado(conexion_revertida):
    ...
    cd = await create_location(
        LocationCreateBody(entity_type="SHIPPER", entity_id=str(shipper),
                           name=f"{PREFIJO} CD EL PEÑON", is_origin_cd=True),
        pool=pool, user=USER,
    )
    await create_location(
        LocationCreateBody(entity_type="SHIPPER", entity_id=str(shipper),
    ...
    solo_cd = await _listar(pool, entity_id=str(shipper), origin_cd="true")
...
async def test_se_le_puede_quitar_el_rol_de_cd(conexion_revertida):
    ...
    creado = await create_location(
        LocationCreateBody(entity_type="SHIPPER", entity_id=str(shipper),
                           name=f"{PREFIJO} Ex CD", is_origin_cd=True),
        pool=pool, user=USER,
    )
    assert creado["is_origin_cd"] is True

    apagado = await patch_location(
        str(creado["id"]), LocationPatchBody(is_origin_cd=False), pool=pool, user=USER,
    )
    assert apagado["is_origin_cd"] is False
