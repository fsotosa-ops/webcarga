...
**Deploy automático post-push**: `Deploy Monitor API to Cloud Run` → success. `Deploy Frontend to Vercel` → **failure** (esperado, no es un bug nuevo). `next run build` no solo tipa, también empaqueta — los ~24 componentes de fase 2 que todavía importan `lib/api/transporters`/`insurance` (borrados) tiran `Module not found`, un error más duro que los 46 de `tsc --noEmit` que ya se habían catalogado. Confirmado con el usuario: **se deja roto a propósito** — se arregla al completar H3 fase 2, no con un stub apurado. `dev` en Vercel/Cloud Run queda con el frontend desactualizado hasta entonces (el backend sí está al día).

#### Próximo paso exacto
1. [ ] Checkpoint H3 fase 2: reescribir los ~24 componentes + 2 páginas de Empresas/Seguros contra los tipos/API nuevos (ya commiteados). Correr `npx tsc --noEmit` en el frontend al arrancar para tener la lista actualizada de archivos pendientes (era 46 al cierre de esta sesión, todos genuinamente Empresas/Seguros — sin fugas a otros módulos). **Prioridad al terminar la fase 2: correr `npm run build` local (no solo `tsc`) y confirmar que pasa antes de pushear** — el build de Vercel encontró un error (`Module not found`) que `tsc --noEmit` no capturó del todo.
